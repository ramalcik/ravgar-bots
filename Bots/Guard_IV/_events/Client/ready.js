...
class ready extends Event {
    ...
    async onLoad() {
        const aris = global.aris = await ariscik.findOne({ guildID: system.Guild.ID })

        setInterval(async () => {
            const arisim = await ariscik.findOne({ guildID: system.Guild.ID })
            const voice = require("@discordjs/voice")
            const channel = client.channels.cache.get(arisim?.botVoiceChannel);
        ...
        _status(
            [
                ...
                { name: `Vich ❤️ Ravgar`, type: 1, url: "https://twitch.tv/ravgar" }, // Twitch'de yayında
