...
Sunucu founderlar : ${aris?.Founders ? `${aris?.Founders.map(x => `<@${x}>`).join(", ")}` : `${message.guild.findEmoji(system.Emojis.Iptal)}`} (\`.config founders <ravgar/ID>\`)
...
Register kanalı : ${aris?.welcomeChannel ? `<#${aris?.welcomeChannel}>` : `${message.guild.findEmoji(system.Emojis.Iptal)}`} (\`.config registerkanal <Kanal/ID>\`)
...
Stagram kanalı : ${aris?.stagramChannel ? `<#${aris?.stagramChannel}>` : `${message.guild.findEmoji(system.Emojis.Iptal)}`} (\`.config stagramkanal <Kanal/ID>\`)
...
Haftalık lider kanalı : ${aris?.weeklyAuthLog ? `<#${aris?.weeklyAuthLog}>` : `${message.guild.findEmoji(system.Emojis.Iptal)}`} (\`.config haftalıkliderlog <Kanal/ID>\`)    
        ...
        if (["underworld", "underworldsystem", "underworldsistem", "underworldban"].some(x => ayar == x)) {
            if (aris?.underWorldSystem == true) {
                ...
                message.channel.send({ content: `${message.guild.findEmoji(system.Emojis.Onay)} ${message.author} başarılı bir şekilde **${ayar}** ayarı ${aris?.underWorldSystem ? "açık" : "kapalı"} olarak ayarlandı!` })
            } else {
