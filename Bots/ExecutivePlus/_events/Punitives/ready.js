    ...
    for (let ceza of Cezalı) {
        ...
        if (uye && ceza.Kalkma && Date.now() >= ceza.Kalkma) {
            ...
            if (CID) {
                ...
                if (!CID && !CID.Isim && !CID.Yas) await uye.setNickname(`${aris?.tags.some(x => uye.user.username.includes(x)) ? aris?.tags[0] : (aris?.unTag ? aris?.unTag : (aris?.tags[0] || ""))} Vich Kayıtsız`)
