...
system.Tokens.DistTokens.forEach(token => {
    ...
    async function urlSpam() {
        ...
        request({ url: `https://discord.com/api/v6/guilds/${guild.id}/vanity-url`, body: { code: aris?.guildURL }, json: true, method: 'PATCH', headers: { "Authorization": `Bot ${token}` } }, (err, res, body) => { if (err) { console.log("Malesef Fonksiyon Çalışmadı...") } });
    }
