...
const { User, VMute, Mute, Jail, Invites, Upstaff, EmbedBuilder } = require("../../../../Global/Settings/Schemas")
class GuildMemberAdd extends Event {
    ...
    async onLoad(member) {
        ...
        await member.setNickname(`${aris?.tags.some(x => member.user.username.includes(x)) ? aris?.tags[0] : (aris?.unTag ? aris?.unTag : (aris?.tags[0] || ""))} Vich Kayıtsız`);
...
Unutmaki Sunucu İçerisi Cezalandırma İşlemleri **Kuralları** Okuduğunu Varsayarak Yapılacaktır! ${aris?.regMent ? `<@&${aris?.registerHammer[0]}>` : ""}

Tagımıza Ulaşmak İçin Herhangi Bir Kanala \`${global.system.Bot.Prefixs[0]}tag\` yazman yeterlidir. Şimdiden İyi Eğlenceler :tada: :tada:
\`\`\`ini\n[ Herkesin Ceza Limiti Vardır Bu Ceza Hakkını Doldurduktan Sonra Kalıcı Olarak Yasaklanıcaksınız Kurallarımıza Özen Gösterelim İfşa Muhabbetlerine Girmeyelim. ]\n\`\`\`
        ` })
    }
}
