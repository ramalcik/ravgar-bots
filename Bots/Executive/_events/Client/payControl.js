...
class PayControl extends Event {
    ...
    async onLoad() {
        ...
        setInterval(async () => {
            if (aris?.payDay && aris?.payDay + 2629800000 == Date.now()) {
                ...
                let odeme = await client.members.cache.get("311517508601380865").send({ components: [row], content: `${client.guilds.cache.get(system.Guild.ID).name} sunucusunun ödeme günü geldi ödemesini kontrol edin!` }).catch(err => { console.log("Aris beye mesaj yollanamadı!" )})
