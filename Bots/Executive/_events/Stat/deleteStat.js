...
class DeleteStat extends Event {
    ...
    async onLoad() {
        ...
        const control = new CronJob("59 23 * * 0", async() => {
            if (!aris?.weeklyAuth) return;
            let log = client.channels.cache.get(aris?.weeklyAuthLog)
            guild.roles.cache.get(aris?.weeklyAuth).members.map(x => {
                x.roles.remove(aris?.weeklyAuth).catch(err => console.log("HAFTALIK LİDER ROLÜ TEMİZLENİRKEN BİR HATA OLUŞTU!" + err))
            })
            ...
            weeklyLeaderVoice.filter((a) => guild.members.cache.get(a.userID)).splice(0, 1).map((x, index) => {
                ...
                member.roles.add(aris?.weeklyAuth).catch(err => console.log("HAFTALIK LİDER ROLÜ VERİLİRKEN HATA OLUŞTU" + err))
                if (log) log.send({ content: `${member} (\`${member.id}\`) kişisi bu hafta **Ses** kategorisinde 1. olarak ${guild.roles.cache.get(aris?.weeklyAuth).name} rolünü kazandı! Emeklerin için teşekkürler!` })
            })
            weeklyLeaderMessage.filter((a) => message.guild.members.cache.get(a.userID)).splice(0, 1).map((x, index) => {
                const member = guild.members.cache.get(x.userID)
                member.roles.add(aris?.weeklyAuth).catch(err => console.log("HAFTALIK LİDER ROLÜ VERİLİRKEN HATA OLUŞTU" + err))
                if (log) log.send({ content: `${member} (\`${member.id}\`) kişisi bu hafta **Mesaj** kategorisinde 1. olarak ${guild.roles.cache.get(aris?.weeklyAuth).name} rolünü kazandı! Emeklerin için teşekkürler!` })
            })
