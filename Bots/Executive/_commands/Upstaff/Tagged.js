...
class Tagged extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        if (!aris?.tags.some(a => member.user.tag.includes(a))) return message.channel.send({ content: `${message.guild.findEmoji(system.Emojis.Iptal)} ${message.author} ${system.Replys.NoTag}` }).sil(20)
        ...
        collector.on('collect', async (interaction) => {
            if (interaction.customId == "onayla") {
                ...
                await Upstaff.updateOne({ guildID: message.guild.id, userID: message.author.id }, { $inc: { coin: aris?.taggedCoin } }, { upsert: true });
            }
