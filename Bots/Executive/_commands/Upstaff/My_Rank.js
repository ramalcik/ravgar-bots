...
class MyRank extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        if (aris?.staffRanks < 0) return message.channel.send({ content: `${message.guild.findEmoji(system.Emojis.Iptal)} verilerim eksik! Lütfen \`ravgar#3210\` ile iletişime geçin!` }).sil(80);
...
`)]
        ...
        collector.on("collect", async (button) => {
            ...
            if (button.customId == "iki") {
                if (yetkim) yetkim.edit({
                    embeds: [new Discord.EmbedBuilder().setAuthor({ name: member.user.tag, iconURL: member.user.avatarURL({ dynamic: true, size: 1024 }) }).setFooter({ text: "Developed By Ravgar.", iconURL: client.user.avatarURL({ dynamic: true }) }).setDescription(`
