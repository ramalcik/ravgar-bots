...
class Register extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        setName = `${aris?.tags.some(x => member.user.tag.includes(x)) ? aris?.tags[0] : (aris?.unTag ? aris?.unTag : (aris?.tags[0] || ""))} ${name}`;
...
${message.guild.findEmoji(system.Emojis.Onay)} ${member} (\`${member.id}\`) kullanıcısı **ERKEK** olarak kayıt edildi!
                ...
                register(member, message.member, name, "man", message, aris)
            }
...
${message.guild.findEmoji(system.Emojis.Onay)} ${member} (\`${member.id}\`) kullanıcısı **KADIN** olarak kayıt edildi!
                ...
                register(member, message.member, name, "woman", message, aris)
            }
...
async function register(member, auth, name, gender, message, aris) {
    let cins;
    if (gender == "man") {
