...
class Ban extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        ...
        if (!aris?.banHammer.some(oku => message.member.roles.cache.has(oku)) && !aris?.foundingRoles.some(oku => message.member.roles.cache.has(oku)) && !message.member.permissions.has(Discord.PermissionsBitField.Flags.Administrator)) return message.channel.send({ content: system.Replys.NoAuth }).sil(20)
...
${message.author} Merhaba! ${member} kişisine hangi ceza işlemini uygulamak istiyorsan lütfen aşağıdan seç! \`${LimitTaslak}\`
            ...
            collector.on('collect', async (interaction) => {
                ...
                if (interaction.values[0] == "bbir") {
                    ...
                    if (!message.member.permissions.has(Discord.PermissionsBitField.Flags.Administrator) && !aris?.foundingRoles.some(oku => message.member.roles.cache.has(oku))) client.banLimit.set(message.author.id, LimitKontrol + 1)
                    await Punitives.find({}).exec(async (err, res) => {
                        ...
                        client.channels.cache.find(x => x.name === "ban-log").send({ embeds: [embed.setDescription(`${member} (\`${member.id}\`) kişisi ${message.author} (\`${message.author.id}\`) tarafından **${sebep}** sebebiyle **${tarihsel(Date.now())}** tarihinde ban türü ceza ile cezalandırıldı!`).setFooter({ text: `Developed By Ravgar. • Ceza Numarası: #${ceza.No}` }).setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true, size: 2048 }) }).setColor("Random")] })
            ...
            collector.on('collect', async (interaction) => {
                ...
                if (interaction.values[0] == "bbir") {
                    ...
                    await Punitives.find({}).exec(async (err, res) => {
                        ...
                        if (member && member.manageable) await member.setRoles(aris?.underWorld).catch(x => client_logger.log("Underworld rolü verilemedi lütfen Rol ID'sini kontrol et.", "caution"));;
                        message.react(message.guild.findEmoji(system.Emojis.Onay))
                    })
                }
                if (interaction.values[0] == "biki") {
