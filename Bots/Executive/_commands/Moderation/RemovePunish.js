            ...
            if (button.customId == "jail") {
                ...
                if (CID) {
                    if (CID.Gender && CID.Name && CID.Age) {
                        ...
                        await member.setNickname(`${member.user.username.includes(aris?.tags[0]) ? aris?.tags[0] : (aris?.unTag ? aris?.unTag : (aris?.tags[0] || ""))} Vich Kayıtsız`)
                    }
                } else {
                    await member.setRoles(aris?.unregisterRoles).catch(x => client._logger.log("Jail komutunda kayıtsız rolü verilemedi oraya bi göz atmanı öneririm.", "caution"));
                    await member.setNickname(`${member.user.username.includes(aris?.tags[0]) ? aris?.tags[0] : (aris?.unTag ? aris?.unTag : (aris?.tags[0] || ""))} c`)
                }
            }
