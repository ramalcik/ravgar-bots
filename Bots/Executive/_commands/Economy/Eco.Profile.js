...
class Daily extends Command {
    ...
    async onRequest(client, message, args, embed, aris) {
        if (aris?.economySystem == false) return message.channel.send({ content: `${message.guild.findEmoji(system.Emojis.Iptal)} \`HATA!\` Bu sunucuda şuanda **Ekonomi Sistemi** kapalı durumdadır!` }).sil(30);
        ...
        const row = new Discord.ActionRowBuilder().addComponents(
            ...
            new Discord.ButtonBuilder().setCustomId("market").setEmoji("925052991994753044").setLabel("Market").setStyle(aris?.ecoProduct.length < 0 ? Discord.ButtonStyle.Danger : Discord.ButtonStyle.Success).setDisabled(aris?.ecoProduct.length < 0 ? true : false),
        )
        if (aris?.Founders.includes(message.member.id) || system.Bot.Roots.includes(message.member.id)) {
            row.addComponents(
        ...
        collector.on('collect', async (interaction) => {
            ...
            if (interaction.customId == "market") {
                if (aris?.ecoProduct.length < 0) return await interaction.reply({ content: `Malesef sistemimde ürün bulunmamakta. Ürünler eklendiğinde tekrar beklerim!`, ephemeral: true })
                let listele = [];
                aris?.ecoProduct.map(async x => {
                    listele.push({ label: x.proName, description: x.proPrice, value: x.proName })
                })

                console.log(listele)
                console.log("------")
                console.log(aris?.ecoProduct)
                ...
                const collector = epro.createMessageComponentCollector({ filter, time: 30000 })
                collector.on('collect', async (interaction) => {
                    if (aris?.ecoProduct.find((e) => e.proName == interaction.values[0])) {
                        let alıncakürün = aris?.ecoProduct.find((e) => e.proName == interaction.values[0])
                    ...
                    collector.on("collect", async (m) => {
                        ...
                        message.channel.send({ content: `Eklenecek ürünün coini \`${urunekleme.coin}\` olarak ayarlandı! Şimdi ise ürünün ismini kanala gönderiniz!` }).then(async (isimbelirleme) => {
                            ...
                            collector.on("collect", async (m) => {
                                ...
                                await ariscik.findOneAndUpdate({ guildID: message.guild.id }, { $push: { ecoProduct: { proName: urunekleme.isim, proPrice: urunekleme.coin } } }, { upsert: true })
