...
const ariscik = global.ariscik = require("../Settings/Ariscik.js");
...
Discord.Guild.prototype.log = async function log(cezano, user, admin, tip, channelName) {
  ...
  if (channel) {
    let embed = new Discord.EmbedBuilder()
      ...
      .setFooter({ text: "Developed By Ravgar" + ` • Ceza Numarası: #${cezano.No}`, iconURL: client.guild.iconURL({ dynamic: true }) })
...
const puni = global.puni = async function (id, type) {
  ...
  if (type == "jail") {
      ...
      return await uye.roles.cache.has(aris?.boosterRolü) ? uye.roles.set([aris?.boosterRolü, aris?.jailRolü]).catch(err => { }) : uye.roles.set([aris?.jailRolü]).catch(err => { });
  }

  if (type == "ban") return await uye.ban({ reason: "Aris Sikmiş Diyolar" }).catch(err => { })
};
